...
<?php
                            ...
                            print($d == '0000-00-00' ?
                                        '-' :
                                        ($d == '9999-12-31' ? 'unbegrenzt' : date('d.m.Y', strtotime($d))));
                            ?></td>
...
<?php
    ...
    <form action="<?php print($_SERVER['PHP_SELF']);
            ?>" method="post" accept-charset="utf-8" class="rcontainer "> 
<?php
...
<?php
            ...
            <fieldset>
                ...
                <p>Trägt trägt den ausgewählten User als neuen Leiter für "<?php print($ally);
                    ?>" ein.</p>
...
<?php
    ...
    <form action="<?php print($_SERVER['PHP_SELF']);
        ...
        <fieldset>
            ...
            <p>Soll "<?php print($name);
            ?>" wirklich als neuer Leiter für die Allianz "<?php print($ally);
...
<?php
        ...
        private function do_it()
        {
            ...
            if (isset($req->overview)) {
                ...
                return ['overview' => 1, 'ally' => $ally, 'forms' => $forms];
            } elseif ($state == 'start') {
                return ['overview' => 1, 'ally' => '-', 'forms' => ['allies', 'member']];
            }

            // now begins the work

            $ret = ['overview' => 1, 'ally' => $req->ally];
            $ret['forms'] = ['member', 'allies'];
