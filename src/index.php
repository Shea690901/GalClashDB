...
function put_add_form($spieler, $allianz)
{
    ...
    ?>" method="post" accept-charset="utf-8"> 
            <fieldset>
                <legend>Kolonie hinzufügen<?php print($session->is_admin() ? ' / löschen' : '');
    ?></legend>
...
<?php
        foreach ($result as $row) {
            ?>
            <tr>
                <td><?php print($allianz != $row->allianz ? $allianz = $row->allianz : '');
            $allianz = $row->allianz;
            ?></td>
                <td><?php print($spieler != $row->name ? $row->name : '');
            $spieler = $row->name;
            ?></td>
